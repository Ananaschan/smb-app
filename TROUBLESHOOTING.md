...
- 仓库已转为公开：GitHub Actions macOS 构建免费不限量（私有仓库每月 2000 分钟被挂起构建耗尽）。
- 当前 HEAD `ec38fec` 代码与上次成功构建 `d5e5cdc` 完全一致，等待构建验证。
...
- 单击屏幕：切换显示/隐藏控制条（底部进度条 + 左上角返回/字幕按钮），播放中 4 秒无操作自动隐藏，暂停时保持显示
- 左右滑动：拖动进度（整屏宽度 = 整个视频时长），中央显示 `当前时间 / 总时长`
- 右侧上下滑动：调节系统音量（`MPVolumeView` 内部 slider），显示 `音量 %`
- 左侧上下滑动：调节屏幕亮度（`UIScreen.brightness`），显示 `亮度 %`
- 底部控制条瘦身：去掉大圆角材质框，改为 播放/暂停小按钮 + 进度条 + 时间文本 单行紧凑布局，配底部渐变保证可读性

第二轮优化（真机反馈）：

- **单击无反应**：`.onTapGesture` 与 `.gesture(DragGesture)` 同视图挂载时拖动抢占点击。
  改为单个 `DragGesture(minimumDistance: 0)` 统一处理：位移 < 12pt 视为单击，控制条区域内的点击不触发切换
- **进度拖动**：拖动中每 0.25s 节流 seek（画面实时跟随），中央时间按拖动位置实时计算，
  底部显示实时进度条，松手做最终 seek
- **音量反馈**：系统音量 HUD 无法移动位置 → 在视图层级挂隐藏 `MPVolumeView` 抑制系统 HUD，
  改用自定义右侧垂直音量条（🔊 + 竖条）；亮度仍用中央 ☀️ + 百分比提示

### 14. iPad 布局调整：共享以缩略图文件夹显示在主视图

按用户要求调整 iPad 布局：

- 三栏改为**两栏**：侧边栏只保留主机选择（可收起），主视图直接显示共享
- 新增 `ShareGridView`：共享以**文件夹缩略图网格**显示在主视图（与文件浏览风格一致），
  点击进入 `BrowseView`（push 导航，返回可回到共享网格）
- 删除 `SplitShareList`（共享列表栏）、`EmptyDetailText`、RootView 的 `selectedShare` 状态
- 切换主机时 `ShareGridView` 按 `server.id` 重建，避免残留上一个主机的导航栈
- 加载失败显示"重试"按钮

### 15. 子文件夹点击后标题变、内容不变（嵌套栈导致）

现象（iPad 两栏布局下复现）：

- 点共享进入文件浏览正常；点里面的**子文件夹**：导航标题变成子文件夹名，但内容仍是上一级
- 根因：iPad 两栏布局后，`BrowseView` 被 push 进外层 `NavigationStack`（共享网格），
  它自己又带一个内层 `NavigationStack(path:)`——**嵌套栈**下内层 value-push 在 iOS 18/26
  渲染了新视图但不触发 `.task`，内容停留在旧状态（与"标题变内容不变"同一机制）

解决：

- **彻底移除 BrowseView 的内层 NavigationStack**：文件夹切换改为纯状态驱动
  （`pathStack.last` 决定当前目录，`.task(id: path)` 随状态变化必然触发）
